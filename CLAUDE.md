...
# Accessibility audits (requires yarn start running separately)
yarn test-a11y -d
```
...
1. Create `src/modules/[module-name]/index.tsx` and supporting files
2. Add an entry to `rollup.config.mjs` input map
3. Rollup generates `assets/[module-name]-bundle.js` and injects the import map into `document_head.hbs`
4. Create `src/modules/[module-name]/translations/en-us.yml` with title and package name header before running i18n extract
...
// Plurals - provide defaultValue.zero, defaultValue.one, etc.
t("my-key", "{{count}} items", { "defaultValue.one": "{{count}} item", count: n });
```
...
Use [conventional commits](https://conventionalcommits.org/). `husky` + `commitlint` enforce this on commit.
...
Types that trigger releases: `feat` (minor), `fix`/`perf`/`revert` (patch). Add `BREAKING CHANGE` in the footer for major releases.

PRs must be approved by @zendesk/vikings. Changes to the UI should be:
- Tested for RTL compatibility
- Accessibility-compliant (WCAG 2.1) — run `yarn test-a11y -d`
- Verified in Chrome, Firefox, Safari, and Edge, including mobile
